...
const variantClasses = {
  error: "text-red-700 border-red-200 [&>svg]:text-red-600",
  warning: "text-amber-700 border-amber-200 [&>svg]:text-amber-600",
  success: "text-green-700 border-green-200 [&>svg]:text-green-600",
  info: "text-blue-700 border-blue-200 [&>svg]:text-blue-600",
} as const;
...
export function AlertMessage({
  ...
  const alertDescription =
    description ||
    (messages.length > 0 ? variantDescription : defaultDescription);

  return (
    <div className={`w-full ${maxWidth} ${className}`}>
      <Alert
        className={`${variantClasses[variant]} flex flex-col items-start gap-2`}
      >
